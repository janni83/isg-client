...
const REGEX_VALUE_CAPACITY = new RegExp(`\\['${HEATING.STANDARD_SETTING.HEATING_SYS_OUTPUT_SIZING.id}'\\]\\['val'\\]='([0-9]{2,3})'`);
...
class HeatingModule {
  ...
  async isActive() {
    ...
    const matchingElements = $('td')
      ...
      .filter((i, columnText) => [TEXT_HEATING, TEXT_COMPRESSOR].includes(columnText));
